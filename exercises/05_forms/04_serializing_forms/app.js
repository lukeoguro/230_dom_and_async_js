class App {
  ...
  handleSubmit(e) {
    ...
    if (e.target.checkValidity()) {
      ...
      p.textContent = this.serializeForm();
      document.querySelector('div.serialized').append(p);
  ...
  serializeForm() {
    ...
    let ccInputs = [...document.querySelectorAll('input[name="credit_card"]')];
    let ccValue = ccInputs.map(input => input.value).join('');
    data.set('credit_card', ccValue);
